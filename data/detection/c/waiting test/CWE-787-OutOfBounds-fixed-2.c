...
int main() {
    ...
    // 修复：在调用前检查 `userIndex` 的有效性
    if (userIndex >= 0 && userIndex < (sizeof(userInput)/sizeof(userInput[0]))) {
        processData(userInput, 5, newValue, userIndex);
    } else {
        printf("Error: Index %d is out of bounds for inputArray of size %lu.\n", userIndex, sizeof(userInput)/sizeof(userInput[0]));
    }
